    ...
    >
      <motion.div variants={itemVariants}>
        <Card className="text-center bg-gradient-to-br from-primary/5 to-secondary/5">
          <h1 className="text-4xl font-bold text-dark-color dark:text-gray-100">{t('chatPage.title', language)}</h1>
          <p className="text-lg text-dark-color/70 dark:text-gray-300 mt-4">{t('chatPage.description', language)}</p>
        </Card>
      </motion.div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
        {(t('chatPage.methods', language) as any[] || []).map((method, index) => (
      ...
      <motion.div variants={itemVariants}>
        <Card>
          <div className="text-center mb-6">
            <h3 className="text-2xl font-bold text-primary">{t('chatPage.formTitle', language)}</h3>
            <p className="text-dark-color/70 dark:text-gray-300 mt-2">{t('chatPage.formDescription', language)}</p>
          </div>
          <ContactForm />
        </Card>
      </motion.div>
